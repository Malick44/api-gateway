package com.nextswift.gateway.repository;
...
public interface UserRepository extends R2dbcRepository<User, String>, UserRepositoryInternal {
    ...
    @Query("INSERT INTO lms_user_authority VALUES(:userId, :authority)")
    Mono<Void> saveUserAuthority(String userId, String authority);

    @Query("DELETE FROM lms_user_authority")
    Mono<Void> deleteAllUserAuthorities();

    @Query("DELETE FROM lms_user_authority WHERE user_id = :userId")
...
class UserRepositoryInternalImpl implements UserRepositoryInternal {
    ...
    public Flux<User> findAllWithAuthorities(Pageable pageable) {
        ...
        return db
            .sql("SELECT * FROM lms_user u LEFT JOIN lms_user_authority ua ON u.id=ua.user_id")
            .map((row, metadata) ->
    ...
    private Mono<User> findOneWithAuthoritiesBy(String fieldName, Object fieldValue) {
        return db
            .sql("SELECT * FROM lms_user u LEFT JOIN lms_user_authority ua ON u.id=ua.user_id WHERE u." + fieldName + " = :" + fieldName)
